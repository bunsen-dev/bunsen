...
Releases are cut **manually** as GitHub Releases — there's no release script or changesets. Publishing a release (`release: published`) triggers CI that fans out automatically. The full checklist lives in [`packaging/README.md`](./packaging/README.md#cutting-a-release); the short version is **bump the public package versions → roll [`CHANGELOG.md`](./CHANGELOG.md)'s `[Unreleased]` section to the version + date (it becomes the Release body) → tag `vX.Y.Z` → publish a full release** (never a prerelease — `install.sh`'s "latest" skips those).

- **CI on `release: published`**: [`release.yaml`](./.github/workflows/release.yaml) builds/signs the standalone `bn` binaries, attaches them + `SHA256SUMS`, and bumps the Homebrew tap + Scoop bucket; [`publish-npm.yaml`](./.github/workflows/publish-npm.yaml) publishes `@bunsen-dev/types` + `@bunsen-dev/sdk` to npm with provenance (the `cli` is binary-distributed, not published).
- **Changelog discipline**: record every public-surface break (see "Break the public surface deliberately" above) under **Changed** / **Removed** in [`CHANGELOG.md`](./CHANGELOG.md) as it lands, so cutting a release is just rolling `[Unreleased]`. The public-vs-internal boundary is documented in [`docs/PACKAGES.md`](./docs/PACKAGES.md).
- Cutting a release is outward-facing and hard to reverse — surface that one's ready and let me publish it rather than doing it yourself.
