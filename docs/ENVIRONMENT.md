...
| `artifactCaptureTimeout` | duration string | `2m`    | Post-run artifact capture (diff, tar export, log retrieval, and the workspace extraction that feeds the dedicated scorer container). A capture step that fails or times out degrades the run's artifacts (`extensions.capture_incomplete` on the manifest) but never fails the completed run — evaluation still happens, with `judge` criteria whose evidence was lost recorded `skipped` rather than scored. |
