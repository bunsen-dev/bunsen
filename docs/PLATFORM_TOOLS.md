...
> **Agent invocation is composed deterministically on the host — no model in the run path.** The runtime's invocation composer (`buildArgvInvocation` in `packages/runtime/src/orchestration.ts`) builds each agent's argv from its declared [`entrypoint`](./AGENT_YAML.md#entrypoint) and the experiment's task prompt, so the invocation is reproducible and comparable across runs. A model is used only at **authoring time** by `bn agents infer-invoke` (host-side, once per agent — see [`packages/agents/src/scaffolder/`](../packages/agents/src/scaffolder/scaffold.ts)) to infer an agent's `entrypoint.invoke` template; that inference runs as part of the `bn` binary. The tools that run inside experiment containers are `scorer`, `supervisor`, `gitignore-filter`, and `proxy-bootstrap`.
