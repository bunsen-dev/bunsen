...
- **Scorer container vs agent container scoring**: By default, scorers run in a separate container with both `/workspace` (final mutable state) and `/workspace-source` (immutable initial snapshot) extracted/mounted from the agent run. With `evaluation.container: agent`, scorers run in the agent's own container via `docker exec`, preserving installed packages, running services, the same `/workspace` + `/workspace-source` contract, and the agent's execution user context. In that mode — and **only** that mode — `/bunsen/verifiers` is mounted into the agent container before the agent runs (Docker cannot add mounts to a running container, so the in-agent scorer needs the files up front), which means verifier assets are visible to the agent. In the default **dedicated** mode the agent container does **not** mount `verifiers/`; the separate scorer container mounts it itself (`buildScorerContainerMounts` in `scorer-container.ts`), so held-out fixtures and golden tests stay hidden from the agent. Files the agent is meant to read go through `workspace.sources`, not `verifiers/`.
