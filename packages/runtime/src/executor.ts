...
export async function executeRun(
  ...
  try {
    ...
    // Verifiers mount (agent container): read-only. Only mounted into the agent
    // container when scorers run *here* (`evaluation.container: agent`), because
    // Docker cannot add mounts to a running container, so the in-agent scorer
    // needs verifier files present before the agent starts. In the default
    // dedicated mode the scorer runs in a *separate* container that mounts
    // verifiers itself (see `buildScorerContainerMounts` in scorer-container.ts),
    // so the agent container must NOT mount them — that would leak held-out
    // fixtures / golden tests to the agent. Files the agent is meant to see go
    // through `workspace.sources`, not verifiers/. (See docs/SCORERS.md.)
    const scorersRunInAgentContainer = experiment.evaluation.container === 'agent';
    if (experiment.verifiersPath && scorersRunInAgentContainer) {
      mounts.push({
